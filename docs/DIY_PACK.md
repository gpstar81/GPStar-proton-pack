...
| N-Filter LED → Pin 46    | Optional Feature (not shown in schematic)<br/>Goes 5V high, is timed to go off at the same time as SMOKE-1 and SMOKE-2. This can be used for another optional LED for the N-Filter during overheat or sustained firing. If you attach a LED to this pin, use the appropriate resistor required for the LED.<br/>**WARNING: DO NOT DRAW MORE THAN 40ma FROM A PIN. Use a transistor setup if you need more power.**  |
