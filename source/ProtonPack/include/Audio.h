...
/**
 ***** IMPORTANT *****
 * If using a WAV Trigger, please make sure you are running the custom GPStar version firmware version 1.40 or higher.
...
const uint8_t i_firing_idle_level = 5; // This adjusts the volume of certain Afterlife / Frozen Empire sounds while the Neutrona Wand is firing.
...
const uint8_t i_brass_idle_level = 40; // This adjusts the volume of certain Afterlife / Frozen Empire Neutrona Wand idle sounds when in Brass Pack mode.
const uint8_t i_slime_bootup_level = 30; // This adjusts the volume of certain Afterlife / Frozen Empire startup sounds when in slime mode.
const uint8_t i_slime_idle_level = 40; // This adjusts the volume of certain Afterlife / Frozen Empire idle sounds when in slime mode.
...
bool setMasterVolumePercentage(uint8_t percentage) {
  ...
  // Convert to decibel value using ProtonPack formula
  i_volume_master = (MINIMUM_VOLUME + i_volume_min_adj) - ((MINIMUM_VOLUME + i_volume_min_adj) * i_volume_master_percentage / 100);
  ...
  else if(i_volume_master < (MINIMUM_VOLUME + i_volume_min_adj)) {
    i_volume_master = (MINIMUM_VOLUME + i_volume_min_adj);
    i_volume_master_percentage = 0;
  }
      ...
      switch(gpstarPack.getSystemTheme()) {
        ...
        default:
          ...
          if(gpstarPack.inStreamMode(SLIME)) {
            ...
            audio.trackGain(S_AFTERLIFE_BEEP_WAND_S4, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_BEEP_WAND_S5, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_RAMP_1, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_RAMP_2, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_RAMP_2_FADE_IN, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_IDLE_1, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_IDLE_2, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_RAMP_DOWN_2, i_volume_effects - i_slime_idle_level);
            audio.trackGain(S_AFTERLIFE_WAND_RAMP_DOWN_2_FADE_OUT, i_volume_effects - i_slime_idle_level);
            ...
            if(b_brass_pack_sound_loop) {
              ...
              audio.trackGain(S_AFTERLIFE_WAND_RAMP_2, i_volume_effects - i_brass_idle_level);
              audio.trackGain(S_AFTERLIFE_WAND_RAMP_2_FADE_IN, i_volume_effects - i_brass_idle_level);
              audio.trackGain(S_AFTERLIFE_WAND_IDLE_1, i_volume_effects - i_brass_idle_level);
              audio.trackGain(S_AFTERLIFE_WAND_IDLE_2, i_volume_effects - i_brass_idle_level);
              audio.trackGain(S_AFTERLIFE_WAND_RAMP_DOWN_2, i_volume_effects - i_brass_idle_level);
              audio.trackGain(S_AFTERLIFE_WAND_RAMP_DOWN_2_FADE_OUT, i_volume_effects - i_brass_idle_level);
          ...
          if(b_brass_pack_sound_loop) {
            if(b_brass_startup_loop) {
              audio.trackGain(S_FROZEN_EMPIRE_BOOT_EFFECT_LOOP, i_volume_effects);
            }
            else {
              audio.trackGain(S_FROZEN_EMPIRE_BRASS_IDLE, i_volume_effects);
            }
          }
        break;
      }
