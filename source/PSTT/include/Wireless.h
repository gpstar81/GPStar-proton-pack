...
bool startAccessPoint() {
  ...
  if(switch_pstt.getStateRaw() == LOW) {
    // If main button is being pressed during boot-up then immediately use the default password.
    b_success = WiFi.softAP(wirelessMgr->getLocalNetworkName().c_str(), wirelessMgr->getDefaultPassword(), 1, false, 6);
