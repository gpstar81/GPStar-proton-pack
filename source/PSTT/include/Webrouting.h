...
void registerWebRoutes() {
  ...
  addBodyRoute(
    ...
    "Saves device settings including WiFi name, target health values, regeneration rates, and damage per power level. Target must be disabled.",
    TAG_CONFIGURATION,
    "JSON object containing device configuration fields for WiFi name, target health, regeneration, and damage values.",
    RESP_SETTINGS_SAVED
  );
  ...
  // Device Control
  addSimpleRoute("/pstt/drop", HTTP_PUT, handleDefeatTarget, "Simulate a defeated target", "Set the PSTT target to the defeated (down) state", TAG_DEVICE_CONTROL);
  addSimpleRoute("/pstt/reset", HTTP_PUT, handleResetTarget, "Reset a defeated target", "Return the PSTT target to the ready (upright) state", TAG_DEVICE_CONTROL);
  addSimpleRoute("/infrared/signal", HTTP_PUT, handleInfraredSignal, "Send an IR signal", "Send an encoded signal via IR transmitter(s)", TAG_DEVICE_CONTROL);
