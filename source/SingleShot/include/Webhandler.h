...
// blaster.html
extern const uint8_t _binary_assets_blaster_html_start[];
extern const uint8_t _binary_assets_blaster_html_end[];
...
void handleBlasterSettings(AsyncWebServerRequest *request) {
  ...
  size_t i_file_len = embeddedFileSize(_binary_assets_blaster_html_start, _binary_assets_blaster_html_end);
  AsyncWebServerResponse *response = request->beginResponse(200, "text/html", _binary_assets_blaster_html_start, i_file_len);
  response->addHeader("Cache-Control", "no-cache, must-revalidate");
  request->send(response); // Serve page content.
}
